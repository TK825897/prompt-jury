...
const result = {
  summary: "Summary",
  ranking: [
    { answerId: "Answer A", rank: 1, overallScore: 80, confidence: 75 },
  ],
  evaluations: [
    {
      answerId: "Answer A",
...
describe("Judge", () => {
  ...
  it("parses JSON wrapped in web UI commentary", () =>
    expect(
      parseJudgeJson(
  ...
  it("calculates weighted scores", () =>
    expect(
      weightedScore(result.evaluations[0].scores, {
        factuality: 30,
        completeness: 20,
        logic: 15,
        actionability: 20,
  ...
  it("creates deterministic anonymous mappings with an injected random source", () => {
    ...
    expect(anonymous.mapping).toEqual({
      "Answer A": "kimi",
      "Answer B": "chatgpt",
    });
  });
  ...
  it("parses standard and SSE completion responses", () => {
    expect(
      parseCompletionResponse(
        JSON.stringify({ choices: [{ message: { content: "standard" } }] }),
      ),
    ).toBe("standard");
    const sse =
