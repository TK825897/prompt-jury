...
import type { EvaluationBundle, EvaluationRun, ResponseDocument, StoredJudgeResult, SynthesizedAnswer } from "./types";
...
interface LabDatabase extends DBSchema {
  ...
  evaluationRuns: { key: string; value: EvaluationRun; indexes: { "by-created-at": string } };
  responses: { key: string; value: ResponseDocument; indexes: { "by-run-id": string } };
  judgeResults: { key: string; value: StoredJudgeResult; indexes: { "by-run-id": string } };
  synthesizedAnswers: { key: string; value: SynthesizedAnswer; indexes: { "by-run-id": string } };
}

const database = openDB<LabDatabase>("multi-llm-response-lab", 4, {
  upgrade(db, oldVersion, _newVersion, transaction) {
    ...
    if (!db.objectStoreNames.contains("synthesizedAnswers")) {
      const store = db.createObjectStore("synthesizedAnswers", { keyPath: "id" });
      store.createIndex("by-run-id", "runId");
    } else if (oldVersion < 4) {
      const store = transaction.objectStore("synthesizedAnswers");
      if (store.indexNames.contains("by-run-id")) store.deleteIndex("by-run-id");
      store.createIndex("by-run-id", "runId");
    }
  },
});
...
export async function saveRunBundle(bundle: EvaluationBundle): Promise<void> {
  ...
  const transaction = db.transaction(["evaluationRuns", "responses", "judgeResults", "synthesizedAnswers"], "readwrite");
  await transaction.objectStore("evaluationRuns").put(bundle.run);
  const responseStore = transaction.objectStore("responses");
  const existing = await responseStore.index("by-run-id").getAllKeys(bundle.run.id);
  await Promise.all(existing.map((key) => responseStore.delete(key)));
  await Promise.all(bundle.responses.map((response) => responseStore.put(response)));
  ...
  const oldSyntheses = await synthesisStore.index("by-run-id").getAllKeys(bundle.run.id);
  await Promise.all(oldSyntheses.map((key) => synthesisStore.delete(key)));
  await Promise.all((bundle.synthesizedAnswers ?? []).map((answer) => synthesisStore.put(answer)));
  await transaction.done;
}

export async function listRuns(): Promise<EvaluationRun[]> {
  const db = await database;
  return (await db.getAllFromIndex("evaluationRuns", "by-created-at")).reverse();
}

export async function getRunBundle(runId: string): Promise<EvaluationBundle | undefined> {
  const db = await database;
  const [run, responses, judgeResult, synthesizedAnswers] = await Promise.all([
  ...
  return run ? { run, responses, judgeResult, synthesizedAnswers } : undefined;
}
...
export async function deleteRun(runId: string): Promise<void> {
  ...
  for (const storeName of ["judgeResults", "synthesizedAnswers"] as const) {
